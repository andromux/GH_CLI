...
class GitCommitTool:
    ...
    def _setup_argument_parser(self):
        ...
        parser.add_argument('--rebase', action='store_true', 
                          help='Usar rebase para integrar cambios remotos')
...
"""
    ...
    def _check_remote_changes(self, branch, remote="origin"):
        """Verifica si hay cambios en el remoto que no están en el local"""
        # Actualizar referencias remotas
        self._exec_command(f"git fetch {remote}", suppress_output=True)
        
        # Comprobar si hay diferencias entre local y remoto
        success, output = self._exec_command(
            f"git rev-list HEAD..{remote}/{branch} --count 2>/dev/null || echo 0", 
    ...
    def _pull_remote_changes(self, branch, remote="origin", use_rebase=False):
        """Integra cambios remotos con los locales"""
        pull_cmd = f"git pull {'--rebase' if use_rebase else ''} {remote} {branch}"
        success, output = self._exec_command(pull_cmd)
        return success
    ...
    def run(self):
        ...
        try:
            ...
            if not self.args.no_push:
                # Comprobar cambios remotos antes de hacer push
                remote_changes = self._check_remote_changes(self.args.branch, self.args.remote)
                
                if remote_changes > 0:
                    print(f"{Colors.YELLOW}[!] Hay {remote_changes} cambios en {self.args.remote}/{self.args.branch} que no están en tu rama local{Colors.ENDC}")
                    print(f"{Colors.YELLOW}[!] Se recomienda integrar estos cambios antes de hacer push{Colors.ENDC}")
                    
                    pull_option = "rebase" if self.args.rebase else "merge"
                    print(f"{Colors.YELLOW}[?] ¿Integrar cambios usando {pull_option}? [S/n]: {Colors.ENDC}", end="")
                    confirm = input().strip().lower()
                    
                    if not confirm or confirm == "s" or confirm == "y":
                        print(f"{Colors.CYAN}[*] Integrando cambios remotos mediante {pull_option}...{Colors.ENDC}")
                        if not self._pull_remote_changes(self.args.branch, self.args.remote, self.args.rebase):
                            print(f"{Colors.YELLOW}[!] Integración de cambios remotos falló. Resuelve los conflictos manualmente.{Colors.ENDC}")
                            return 1
                    else:
                        print(f"{Colors.YELLOW}[!] Continuando sin integrar cambios. El push puede ser rechazado.{Colors.ENDC}")
                ...
                if not self._exec_command(push_cmd)[0]:
                    # Si el push falla, ofrecer integrar cambios remotos y reintentar
                    print(f"{Colors.YELLOW}[!] Push rechazado. ¿Intentar integrar cambios remotos y reintentar? [S/n]: {Colors.ENDC}", end="")
                    confirm = input().strip().lower()
                    if not confirm or confirm == "s" or confirm == "y":
                        print(f"{Colors.CYAN}[*] Integrando cambios remotos...{Colors.ENDC}")
                        if not self._pull_remote_changes(self.args.branch, self.args.remote, self.args.rebase):
                            print(f"{Colors.YELLOW}[!] Integración de cambios remotos falló. Resuelve los conflictos manualmente.{Colors.ENDC}")
                            return 1
                        
                        print(f"{Colors.CYAN}[*] Reintentando push...{Colors.ENDC}")
                        if not self._exec_command(push_cmd)[0]:
                            return 1
                    else:
                        print(f"{Colors.YELLOW}[!] Push cancelado{Colors.ENDC}")
